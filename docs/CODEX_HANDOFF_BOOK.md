...
- The top hook card should visually match the reference: white rounded rectangle at y=336 on a 1080x1920 render, centered and content-hugging instead of fixed full-width. The reference maximum is x=99-984 including shadow; valid card widths are 520-890px depending on the longest line, and height hugs the text block up to the reference max height of 157px. Text starts 35px inside the card at visual y=356; use Arial Bold 52px with 64px emoji and a 10px two-line gap. Do not regress to the old small pill-style TikTok ExtraBold overlay or the old 48px fallback.
