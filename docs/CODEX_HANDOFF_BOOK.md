...
- The top hook card should visually match the reference: author the white rounded rectangle in 720x1280 source-design space, then upscale it into the 1080x1920 render with the video. Do not draw the hook as crisp native-1080 text. In final output the card sits around y=336, centered and content-hugging instead of fixed full-width. The reference maximum is x=99-984 including shadow; valid card widths are roughly 520-890px depending on the longest line, and height hugs the text block up to about 157px. In source space on macOS, use the system font (`SFNS.ttf`) Semibold variation at 38px, 40px emoji, visual text y=238, 23px text padding, a 10px two-line gap, and a 14px card radius. After upscale this lands around 57px text, 60px emoji, y=357 text, a 15px gap, and about a 21px corner radius. TikTok Sans SemiBold is fallback for non-mac rebuilds; Arial Bold is emergency fallback only. Do not regress to native-1080 Arial, the old small pill-style TikTok ExtraBold overlay, or the old 48px fallback.
