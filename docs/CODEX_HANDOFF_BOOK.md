...
- The top hook card should visually match the reference: white rounded rectangle at y=336 and height=157 on a 1080x1920 render, centered and content-hugging instead of fixed full-width. The reference maximum is x=99-984 including shadow; valid card widths are 520-890px depending on the longest line. Text starts 35px inside the card at visual y=356; use Arial Bold 48px with 64px emoji and an 8px two-line gap. Do not regress to the old small pill-style TikTok ExtraBold overlay or the old fixed-width 52px card.
