...
- The top hook card should visually match the reference: author the white rounded rectangle in 720x1280 source-design space, then upscale it into the 1080x1920 render with the video. Do not draw the hook as crisp native-1080 text. In final output the card sits around y=336, centered, and reaches the reference max width for normal two-line hooks instead of shrinking into an indented pill. The reference maximum is x=99-984 including shadow; short one-line hooks may hug content from roughly 520-890px, and height hugs the text block up to about 157px. In source space, use bundled TikTok Sans SemiBold at 35px, 40px emoji, visual text y=238, 23px text padding, a 10px two-line gap, and a 14px card radius. After upscale this lands around 52px text, 60px emoji, y=359 text, a 15px gap, and about a 21px corner radius. Avenir Next Demi Bold and SFNS Semibold are fallbacks for machines without the bundled TikTok font; Arial Bold is emergency fallback only. Do not regress to native-1080 Arial, the old small pill-style TikTok ExtraBold overlay, the old 48px fallback, or a narrow card that indents long two-line hooks.
