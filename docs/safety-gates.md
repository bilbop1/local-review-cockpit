...
- Social posting unless the review kit is approved, the Upload-Post/provider key is configured outside the repo, the exact local Upload-Post profile is configured, selected-platform warm-up is complete, live mode is enabled, and either local auto-post has armed the scheduled job or the GUI final confirmation is given
- Payout submission
- Account connection
- Account rebrand
...
- Manual publishing prep and Upload-Post package checks only for approved non-demo kits after campaign gate qualification
...
- Publish status proof from `/api/publish/status` showing provider readiness, auto-post state, runway, and locked profile without exposing API keys.
