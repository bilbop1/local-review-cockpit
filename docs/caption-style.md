...
- Top hook geometry is part of the standard: author the white rounded card in a 720x1280 source-design space and upscale it into the 1080x1920 render with the video. Do not draw the top card as crisp native-1080 text. In final 1080 output, the card sits at y=336 and is centered. Long hooks reach the reference max card width of x=99-984 including shadow; shorter hooks shrink to visible text plus reference padding rather than carrying dead white space or collapsing into a tiny app-style pill. In source space, use bundled TikTok Sans Bold at 34px, 40px emoji raised 5px above the text visual top, visual text y=237, 22px text inset, a 10px two-line gap, visual text width plus roughly 46px of card padding, a 330px source-space minimum card width, and a 14px source-space card radius; after upscale, apply the top-anchored 3.4% visible-card vertical stretch and do not apply the old half-resolution bicubic softening pass. This lands around 51px text, 60px emoji, y=357-358 text, a 15px gap, and the taller white-card fill seen in the reference while keeping darker glyph edges. TikTok Sans SemiBold, Avenir Next Demi Bold, and `SFNS.ttf` Semibold are fallbacks for machines without the bundled bold font; Arial Bold is emergency fallback only, not the production top-card face. The card height hugs the text block instead of leaving a loose bottom gutter.
