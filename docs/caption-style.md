...
- Top hook geometry is part of the standard: author the white rounded card in a 720x1280 source-design space and upscale it into the 1080x1920 render with the video. Do not draw the top card as crisp native-1080 text. In final 1080 output, the white card sits at x=99-980, y=336-493 and is centered. Shorter hooks shrink to visible text plus reference padding rather than carrying dead white space or collapsing into a tiny app-style pill. In source space, use bundled TikTok Sans Bold at 34px, pure black text, 40px emoji raised 5px above the text visual top, white-card y=223, visual text y=237, a 21px text inset, a 548px source-space text line cap, a 10px two-line gap, visual text width plus roughly 46px of card padding, a 587px source-space max card width, a 330px source-space minimum card width, and a 14px source-space card radius; after upscale, apply the top-anchored 5.2% visible-card vertical stretch and do not apply the old half-resolution bicubic softening pass. This lands around 51px text, 60px emoji, text y=357-358, a 15px gap, and a white-card fill whose decoded bbox matches the reference while keeping darker glyph edges. The 548px line cap is required so live hooks do not crowd the right card edge. TikTok Sans SemiBold, Avenir Next Demi Bold, and `SFNS.ttf` Semibold are fallbacks for machines without the bundled bold font; Arial Bold is emergency fallback only, not the production top-card face. The card height hugs the text block instead of leaving a loose bottom gutter, and audits must measure the white card itself instead of the drop shadow.
...
- Use `script/audit_top_card_reference.py` for exact reference geometry and `script/audit_live_top_cards.py --refresh` for decoded `review.mp4` proof. Do not judge top-card parity from sidecar text or regenerated overlays alone.
