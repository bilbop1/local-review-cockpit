...
- Top hook geometry is part of the standard: author the white rounded card in a 720x1280 source-design space and upscale it into the 1080x1920 render with the video. Do not draw the top card as crisp native-1080 text. In final 1080 output, the card sits at y=336, is centered, and hugs the longest line instead of becoming a dead full-width banner. The reference maximum card is x=99-984 including shadow, with content-hugging widths allowed from roughly 520-890px. In source space on macOS, use the system font (`SFNS.ttf`) Semibold variation at 38px, 40px emoji, visual text y=238, 23px left/right text padding, a 10px two-line gap, and a 14px source-space card radius; after upscale this lands around 57px text, 60px emoji, y=357 text, a 15px line gap, and about a 21px corner radius. TikTok Sans SemiBold is fallback for non-mac rebuilds; Arial Bold is emergency fallback only, not the production top-card face. The card height hugs the text block up to the reference max height of about 157px.
