...
- Top hook geometry is part of the standard: author the white rounded card in a 720x1280 source-design space and upscale it into the 1080x1920 render with the video. Do not draw the top card as crisp native-1080 text. In final 1080 output, the card sits at y=336, is centered, and all normal two-line hooks use the same reference-width white card instead of shrinking into an indented pill. The reference max card is x=99-984 including shadow; only true one-line hooks may hug content. In source space, use bundled TikTok Sans Bold at 34px, 40px emoji raised 5px above the text visual top, visual text y=238, 23px left/right text padding, a 10px two-line gap, and a 14px source-space card radius; after upscale, apply the tiny top-anchored 1.5% visible-card vertical stretch and then the half-resolution bicubic softening pass. This lands around 51px text, 60px emoji, y=358 text, a 15px line gap, and the tighter/taller white-card fill seen in the reference. TikTok Sans SemiBold, Avenir Next Demi Bold, and `SFNS.ttf` Semibold are fallbacks for machines without the bundled bold font; Arial Bold is emergency fallback only, not the production top-card face. The card height hugs the text block instead of leaving a loose bottom gutter.
