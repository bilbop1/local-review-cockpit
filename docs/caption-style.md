...
- Top hook geometry is part of the standard: author the white rounded card in a 720x1280 source-design space and upscale it into the 1080x1920 render with the video. Do not draw the top card as crisp native-1080 text. In final 1080 output, the card sits at y=336, is centered, and reaches the reference max width for normal two-line hooks instead of shrinking into an indented pill. The reference max card is x=99-984 including shadow, with shorter one-line hooks allowed to hug content from roughly 520-890px. In source space, use bundled TikTok Sans SemiBold at 35px, 40px emoji, visual text y=238, 23px left/right text padding, a 10px two-line gap, and a 14px source-space card radius; after upscale this lands around 52px text, 60px emoji, y=359 text, a 15px line gap, and about a 21px corner radius. Avenir Next Demi Bold and `SFNS.ttf` Semibold are fallbacks for machines without the bundled TikTok font; Arial Bold is emergency fallback only, not the production top-card face. The card height hugs the text block up to the reference max height of about 157px.
