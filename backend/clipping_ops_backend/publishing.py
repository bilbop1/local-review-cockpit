...
def create_publish_job(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    now = db.utc_now()
    status = "queued" if mode == "dry_run" else "awaiting_confirmation"
    db.execute(
        """
        ...
        (
            ...
            scheduled_at,
            status,
            "queued-for-hermes" if mode == "dry_run" else "awaiting-final-confirmation",
            now,
            now,
        ),
    )
    hermes_intent = "publish_dry_run" if mode == "dry_run" else ""
    if hermes_intent:
